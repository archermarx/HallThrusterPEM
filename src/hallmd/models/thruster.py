...
def _convert_to_julia(pem_data: dict, julia_data: dict, pem_to_julia: dict):
    ...
    for pem_key, value in pem_data.items():
        if pem_key not in pem_to_julia:
            raise KeyError(
                f"Cannot convert PEM data variable {pem_key} since it is not in the provided conversion map"
            )
        ...
        for i, key in enumerate(julia_path[:-1]):
            if isinstance(pointer, dict) and not pointer.get(key):
                pointer.setdefault(
                    key, {} if isinstance(julia_path[i + 1], str) else []
                )
            if isinstance(pointer, list) and len(pointer) <= key:
                pointer.extend(
                    [
                        {} if isinstance(julia_path[i + 1], str) else []
                        for _ in range(key - len(pointer) + 1)
                    ]
                )
            pointer = pointer[key]
...
def _default_model_fidelity(
    model_fidelity: tuple, json_config: dict, cfl: float = 0.2
) -> dict:
...
def _format_hallthruster_jl_input(
    ...
    avg_start_time = json_config["postprocess"].get(
        "average_start_time", 0.5 * duration
    )
...
def run_hallthruster_jl(
    json_input: dict | str | Path,
    jl_env: str | Path = None,
    jl_script: str | Path = None,
    **kwargs,
) -> dict:
    ...
    if "output_file" in json_input.get("postprocess", {}):
        ...
        if json_input.get("input"):
            ...
            json_input["input"]["postprocess"]["output_file"] = str(
                output_file.resolve()
            )
        else:
...
def hallthruster_jl(
    ...
    # Run Julia
    t1 = time.time()
    sim_results = run_hallthruster_jl(
        json_data, jl_env=jl_environment, jl_script=julia_script, **run_kwargs
    )
    t2 = time.time()
    ...
    if thrust < 0 or beam_current < 0:
        raise ValueError(
            f"Exception due to non-physical case: thrust={thrust} N, "
    ...
    if output_path is not None:
        ...
        thruster_outputs["output_path"] = output_file.relative_to(
